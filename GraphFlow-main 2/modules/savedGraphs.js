...
export function setupSavedGraphs(user) {
    ...
    const newSaveGraphBtn = saveGraphBtn.cloneNode(true);
    saveGraphBtn.parentNode.replaceChild(newSaveGraphBtn, saveGraphBtn);

    if (user) {
        savedGraphsSection.style.display = 'block';
        loadSavedGraphs();

        newSaveGraphBtn.addEventListener('click', () => {
...
function displayGraphs(graphs) {
    ...
    graphs.forEach(graph => {
        ...
        graphElement.querySelector('.load-graph').addEventListener('click', () => {
            loadGraph(graph);
            //aggiunge listener per chiusura sidebar nel responsive
...
function loadGraph(graph) {
    ...
    // Calcola il centro e il raggio per l'animazione (come in RandomGraph)
    const pan = cy.pan();
    const zoom = cy.zoom();
    ...
    // Aggiungi i nodi con animazione
    graph.nodes.forEach(node => {
        // Calcola posizione di partenza per l'animazione
        const angle = 2 * Math.PI * Math.random();
        const startX = centerX + radius * Math.cos(angle);
        const startY = centerY + radius * Math.sin(angle);

        // Aggiungi il nodo nella posizione di partenza
        ...
        // Anima il nodo verso la sua posizione finale
        newNode.animate({
            position: node.position,
            duration: 800,
            easing: 'ease-in-out'
